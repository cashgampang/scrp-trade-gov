...
function mapTradeEvent(){
	...
	$('#tradeevent svg use').addClass('d-none');
	var url = getContextPath() + "/webHP?requestType=ApplicationRH&actionVal=getHomepageEventList&screenId=1000";
	
	ajax.post(url, null, function(res) {
		var tradeEvent = res.eventMasterDTOs;
		$.each(tradeEvent, function (i) {
			
			$("[data-country-id="+tradeEvent[i].country.key+"][data-zone="+tradeEvent[i].zone.value+"]").css('fill','#0f66fb');
			$("[data-country-id="+tradeEvent[i].country.key+"][data-zone="+tradeEvent[i].zone.value+"]").attr('data-event-id',tradeEvent[i].eventId);
			$("[data-country-id="+tradeEvent[i].country.key+"][data-zone="+tradeEvent[i].zone.value+"]").attr('data-event-name',tradeEvent[i].nameOfEvent);
			$("[data-country-id="+tradeEvent[i].country.key+"][data-zone="+tradeEvent[i].zone.value+"]").attr('data-country-flag',tradeEvent[i].countryFlagPath);
			$("[data-country-id="+tradeEvent[i].country.key+"][data-zone="+tradeEvent[i].zone.value+"]").attr('data-event-location',tradeEvent[i].location);
			$("[data-country-id="+tradeEvent[i].country.key+"][data-zone="+tradeEvent[i].zone.value+"]use").removeClass('d-none');
	...
	$(document).on('click','#tradeevent use',function(){
		...
		event = event + '-' + eventName.replaceAll(' ','-').replaceAll('---','-').replaceAll('--','-').replaceAll(/[!@#$%^&*`"\'(),.?:{}|<>]/g,'').toLowerCase();
	...
	$("use").mousemove(function(evnt){
		...
		var x = evnt.offsetX;
		var y = evnt.offsetY;
		$('.trade-map-section .map-demo').css('left',x+ "px");
		$('.trade-map-section .map-demo').css('top',y + "px");
		var eventName = $(this).data('event-name');
		var eventLocation = $(this).data('event-location');
		var country_flag = $(this).data('country-flag');
	
		$('.map-demo p').text('');
		$('.map-demo p').append("<span class='font-weight-bold pb-1'>"+eventLocation+"</span><br><span>"+eventName+"</span>");
...
function mapTradeMarket(){
	...
	var url = "webHP?requestType=ApplicationRH&actionVal=getHomepageMarketList&screenId=1000";
	
	ajax.post(url, null, function(res) {
		var marketList = res.marketList;
		$.each(marketList, function (i) {
			$("[data-country-id="+marketList[i].country.key+"][data-zone="+marketList[i].zone.value+"]").css('fill','#0f66fb');
			$("[data-country-id="+marketList[i].country.key+"][data-zone="+marketList[i].zone.value+"]").attr('data-country-name',marketList[i].country.value);
			$("[data-country-id="+marketList[i].country.key+"][data-zone="+marketList[i].zone.value+"]").attr('data-country-flag',marketList[i].countryFlagPath);
			$("[data-country-id="+marketList[i].country.key+"][data-zone="+marketList[i].zone.value+"]use").removeClass('d-none');
		});
	});
...
$(document).on('click','.PolicyPage',function(){
 ...
 function tradeCount() {
		...
		var url = getContextPath() + "/webHP?requestType=ApplicationRH&actionVal=getCommonCountTcp&screenId=1000";
 ...
 function getHomepageEventList() {
		...
		ajax.post(url, null, function(res) {
			...
			$.each(tradeEvent, function (i) {
			    var templateString = '<div class="services-outer item">'+
				'<div class="services-inner">'+
				'<a href="javascript:;" class="trade-event-card">'+
				'<div class="trade-event-up-face trade-event-img-up">'+
				'<img src="'+tradeImgPath+tradeEvent[i].eventImgaeForList+'" alt="'+tradeEvent[i].nameOfEvent+'" class="img-fluid">'+
					'<p class="font-22 text-black pt-2 trade-event-slidertitle">'+tradeEvent[i].nameOfEvent+'</p></div>'+
					'<div class="trade-event-up-face trade-event-up-desc">'+
						...
						'<p class="mb-0 font-14">'+tradeEvent[i].venueAddress+'</p></div></div>'+
						'<div class="text-black trade-event-up-icon-desc" style="display: flex; align-items: baseline;">'+
						'<div><i class="fa fa-globe pr-2 font-22" aria-hidden="true"></i></div><div>'+
						'<p class="mb-2 font-18">Date</p>'+
						'<p class="mb-0 font-14">'+tradeEvent[i].eventStartDate+'</p>'+
 ...
 function gethomepageMiniSlider() {
		...
		var url = getContextPath() + "www.trade.gov.in/webHP?requestType=ApplicationRH&actionVal=getHomepageEventList&screenId=1000";
	...
	$.each(tradeEvent, function (i) {
		var eventName = tradeEvent[i].eventId + '-' + tradeEvent[i].nameOfEvent.replaceAll(' ','-').replaceAll('---','-').replaceAll('--','-').replaceAll(/[!@#$%^&*`"\'(),.?:{}|<>]/g,'').toLowerCase();
				    var templateString = '<div class="carousel-item">'+
					'<a href="javascript:;" onclick="getHomeEventDetails(\''+eventName+'\');" class="eventDetails card ">'+
					'<div class="custom-img-break" style="height: 15rem;"><img src="'+tradeImgPath+tradeEvent[i].eventImgaeForList+'" alt="'+tradeEvent[i].nameOfEvent+'" class="w-100 dashboard-slider-img"> </div>'+
					'<div class="event-card-details-div d-flex">'+
						'<div class="w-75 event-desc-details">'+
						'<h2 class="event-title-name">'+tradeEvent[i].nameOfEvent+'</h2>'+
							'<p class="event-title-address">'+tradeEvent[i].venueAddress+'</p></div>'+
							'<div class="w-25 event-detail-date">'+
							'<div class="event-bg-date">'+
							'<p class="event-month"><span>'+tradeEvent[i].eventMonth+'</span></p>'+
							'<p class="event-date"><span>'+tradeEvent[i].eventDay+'</span></p>'+
							'</div></div></div></a></div>';
							$('#countrySlider .carousel-inner').append(templateString);
							$('#countrySlider .carousel-inner .carousel-item:first-child').addClass('active');
						})
					});
				} 
 ...
 var getHomeMarketDetails = (function(countryId){
		var url='webHP?requestType=ApplicationRH&actionVal=marketAccessLoadPage&screenId=1001&menuCode=800000&countryId='+ countryId + '&auditUSFlag=true';
...
$(document).on("click",".market-card-home",function(){
    ...
    window.location.href = getContextPath() + searchPage + value.replaceAll(' ','-').replaceAll(/[!@#$%^&*`"\'(),.?:{}|<>]/g,'').toLowerCase();
});
